...
async function fetchFeedDetails() {
    try {
        ...
        if (data && data.data) {
            ...
            document.getElementById('contents').value = feedDetail.contents;
            const contentsDate = feedDetail.contentsDate;
            console.log(contentsDate);
            const year = contentsDate[0];
            const month = String(contentsDate[1]).padStart(2, '0');
            const day = String(contentsDate[2]).padStart(2, '0');
            const formattedDate = `${year}-${month}-${day}`;
            console.log(formattedDate);
            // input 필드에 날짜 설정
            document.getElementById('feed-date').value = formattedDate;
            document.getElementById('photoSpotName').value = feedDetail.photoSpotName;
            document.getElementById('realAddress').value = feedDetail.address;

            if (feedDetail.image) {
                document.getElementById('feed-image').src = `/feeds_image/${feedDetail.image}`;
            }
        }
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', fetchFeedDetails);